...
while [ "$#" -gt 0 ]; do
    case "$1" in
        ...
        *) echo "usage: $0 [--home PATH] [--agents codex|claude|codex,claude] [--profile PATH]" >&2; exit 2 ;;
    esac
done
...
if HOME="$home" PAC_ROOT="$repo" "$pac" --json --home "$home" \
    --hosts "$pac_hosts" status > "$status_json" 2> "$tmp/status.err"; then
    status_paths_valid=1
    if ! node - "$status_json" <<'NODE'
const fs = require('node:fs');
const envelope = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));
const data = envelope.data;
if (!envelope.ok || !data?.ok) throw new Error('PAC status is not healthy');
