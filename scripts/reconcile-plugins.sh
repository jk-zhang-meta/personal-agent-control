...
source_pending_install() {
    [ "$mode" = preflight ] &&
        [ ! -e "$source_parent/$1" ] && [ ! -L "$source_parent/$1" ] &&
        ! marketplace_was_owned "$1"
}
...
while IFS="$(printf '\t')" read -r marketplace acquisition source ref commit tree; do
    ...
    elif source_pending_install "$marketplace"; then
        continue
...
while IFS="$(printf '\t')" read -r plugin marketplace _acquisition _source _ref _commit _tree _version _targets bundled _rest; do
    ...
    [ "$bundled" != - ] || continue
    source_pending_install "$marketplace" && continue
    directory="$source_parent/$marketplace"
    remaining=$bundled
...
assert_all_installed_managed() {
    ...
    if [ "$allow_prior_owned" = true ] && [ -f "$owned" ]; then
        awk -F '\t' -v host="$host_name" \
            '$0 !~ /^#/ && NF && ($3 == host || $3 == "codex,claude") { print $2 }' \
            "$owned" >> "$tmp/$host_name-managed-marketplaces"
    fi
...
for (const row of rows) {
    ...
    if (typeof id !== 'string' || id.length === 0) {
        console.error(`invalid ${host} Plugin inventory row: missing id`);
        process.exit(2);
    }
