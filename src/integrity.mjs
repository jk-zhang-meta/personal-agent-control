...
const CATALOG_FILES = ['catalog/capabilities.jsonl', 'catalog/taxonomy.json'];
...
export async function verifyCanonicalPayload(context) {
  ...
  for (const [index, line] of text.split(/\r?\n/u).entries()) {
    ...
    if (!match[2].startsWith('payload/skills/') && !CATALOG_FILES.includes(match[2])) {
      throw new PacError('SOURCE_INTEGRITY_INVALID', `Integrity manifest contains an unmanaged path: ${match[2]}`);
    }
    expected.set(match[2], match[1]);
  }
  const actualPaths = [...await payloadInventory(context.root), ...CATALOG_FILES].sort();
