...
Profile and CLI semantics were checked against the implementation on
...
- The managed `AGENTS.md` contains one generic instruction to read
  `~/.config/personal-agent-control/profile-bootstrap.md` when a Profile
...
Native Plugin desired state is also merged before host filtering. The Profile
